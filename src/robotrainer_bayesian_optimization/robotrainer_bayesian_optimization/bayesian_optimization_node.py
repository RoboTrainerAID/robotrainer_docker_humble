...
class BayesianOptimizationNode(Node):
    ...
    def __init__(self, is_ground_truth=False):
        ...
        if not self.is_ground_truth:
            ...
            self.get_logger().info("Ground truth mode: No Bayesian Optimization, only scenario creation")
            random.seed(32)
            self.random_trial_indices = random.sample(range(0, 101, 5), k=21)  # 0,5,10,...,100
            self.next_index = 0
            ...
            self.get_logger().info("Successfully started robotrainer_bayesian_optimization in ground truth mode") 
